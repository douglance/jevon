...
one. Compare `precision` against `base_rate`: a gate no better than the base \
rate is not a gate. For a choice question the gap between `accuracy` and \
`accuracy_when_confident` is what filtering on confidence would buy you. If a \
question scores near chance, first ask whether its answer is present in the \
text at all — a question about what will happen next, or about the state of a \
machine, cannot be answered from the item however well it is worded.";
...
fn examples() -> Vec<Example> {
    vec![
        ...
        Example {
            command: "--labels-file graded.json --choice billing --choice technical \
                      --noul \"What is this about?\""
                .to_owned(),
            description: Some("Measure a choice, and what confidence filtering buys".to_owned()),
        },
    ]
}
